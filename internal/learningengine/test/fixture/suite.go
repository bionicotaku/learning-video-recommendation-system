...
func applyLearningEngineSchema(ctx context.Context, db execer) error {
	sqlFiles := []string{
		filepath.Join(repoRoot(), "internal", "learningengine", "infrastructure", "persistence", "schema", "000000_external_refs.sql"),
	}

	migrations, err := migrationFiles(filepath.Join(repoRoot(), "internal", "learningengine", "infrastructure", "migration"))
	if err != nil {
		return err
	}
