...
const (
	...
	templateDBName = "learningengine_template"
)
...
func OpenSuite() (*Suite, error) {
	baseDir, err := os.MkdirTemp("", "learningengine-integration-*")
	if err != nil {
...
func (s *Suite) prepareTemplateDatabase() error {
	...
	if err := applyLearningEngineSchema(context.Background(), templatePool); err != nil {
		return err
	}
	return nil
}
...
func (s *Suite) nextDatabaseName() string {
	...
	defer s.mu.Unlock()
	s.nextDBID++
	return fmt.Sprintf("learningengine_test_%d", s.nextDBID)
}
...
func applyLearningEngineSchema(ctx context.Context, db execer) error {
	sqlFiles := []string{
		filepath.Join(repoRoot(), "internal", "learningengine", "reducer", "infrastructure", "persistence", "schema", "000000_external_refs.sql"),
	}

	migrations, err := migrationFiles(filepath.Join(repoRoot(), "internal", "learningengine", "reducer", "infrastructure", "migration"))
	if err != nil {
		return err
	}
	sqlFiles = append(sqlFiles, migrations...)
