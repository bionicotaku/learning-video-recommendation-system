...
func TestTimePointerFromPGNormalizesToUTC(t *testing.T) {
	localTime := time.Date(2026, 5, 15, 10, 0, 0, 0, time.FixedZone("PDT", -7*60*60))

	got := mapper.TimePointerFromPG(pgtype.Timestamptz{Time: localTime, Valid: true})

	if got == nil {
		t.Fatalf("TimePointerFromPG() = nil, want value")
	}
	if got.Location() != time.UTC {
		t.Fatalf("TimePointerFromPG() location = %v, want UTC", got.Location())
	}
	if !got.Equal(localTime) {
		t.Fatalf("TimePointerFromPG() = %v, want same instant as %v", *got, localTime)
	}
}
