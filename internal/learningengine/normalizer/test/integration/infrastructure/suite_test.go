...
func TestMain(m *testing.M) {
	...
	if err != nil {
		_, _ = fmt.Fprintf(os.Stderr, "open learningengine normalizer integration suite: %v\n", err)
		os.Exit(1)
	}
	sharedSuite = suite
	...
	if err := suite.Close(); err != nil {
		_, _ = fmt.Fprintf(os.Stderr, "close learningengine normalizer integration suite: %v\n", err)
		if code == 0 {
			code = 1
		}
	}
...
func testDB(t *testing.T) *fixture.TestDatabase {
	t.Helper()
	if sharedSuite == nil {
		t.Fatal("learningengine normalizer integration suite not initialized")
	}
