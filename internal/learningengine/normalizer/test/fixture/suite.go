...
const (
	...
	templateDBName = "learningengine_normalizer_template"
)
...
func OpenSuite() (*Suite, error) {
	baseDir, err := os.MkdirTemp("", "learningengine-normalizer-integration-*")
	if err != nil {
...
func (s *Suite) prepareTemplateDatabase() error {
	...
	if err := applyNormalizerSchema(context.Background(), templatePool); err != nil {
		return err
	}
	return nil
}
...
func (s *Suite) nextDatabaseName() string {
	...
	defer s.mu.Unlock()
	s.nextDBID++
	return fmt.Sprintf("learningengine_normalizer_test_%d", s.nextDBID)
}
...
func applyNormalizerSchema(ctx context.Context, db execer) error {
	sqlFiles := []string{
		filepath.Join(repoRoot(), "internal", "learningengine", "normalizer", "infrastructure", "persistence", "schema", "000000_external_refs.sql"),
	}

	analyticsMigrations, err := migrationFiles(filepath.Join(repoRoot(), "internal", "analytics", "infrastructure", "migration"))
	if err != nil {
		return err
	}
	...
	sqlFiles = append(sqlFiles, learningEngineMigrations...)
