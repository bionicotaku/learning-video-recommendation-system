...
func TestGenerateRecommendationsPrefersDueReviewWhenQuotaIsTight(t *testing.T) {
	...
	unitIDs, err := createTestCoarseUnits(ctx, pool, 2)
	if err != nil {
	...
	uc := newGenerateUseCase(pool, sqlcgen.New(pool))
	result, err := uc.Execute(ctx, generateCmd(userID, 1, now))
	if err != nil {
...
func TestGenerateRecommendationsSuppressesRecentlyRecommendedNew(t *testing.T) {
	...
	if err := insertServingState(ctx, pool, userID, unitIDs[0], now.Add(-25*time.Hour)); err != nil {
		t.Fatalf("insertServingState(old) error = %v", err)
	}
	if err := insertServingState(ctx, pool, userID, unitIDs[1], now.Add(-1*time.Hour)); err != nil {
