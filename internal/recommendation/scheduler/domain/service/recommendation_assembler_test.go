...
func TestRecommendationAssemblerBuildsPredictableBatch(t *testing.T) {
	...
	priorityZero := []appquery.ScoredReviewCandidate{
		{
			Candidate: appquery.ReviewCandidate{
				State: model.UserUnitState{CoarseUnitID: 1, Status: enum.UnitStatusLearning, TargetPriority: 0.9},
				Unit:  model.LearningUnitRef{CoarseUnitID: 1, Kind: enum.UnitKindWord, Label: "hood"},
			},
			Score:       0.91,
	...
	scoredReviews := []appquery.ScoredReviewCandidate{
		{
			Candidate: appquery.ReviewCandidate{
				State: model.UserUnitState{CoarseUnitID: 2, Status: enum.UnitStatusReviewing, TargetPriority: 0.8},
				Unit:  model.LearningUnitRef{CoarseUnitID: 2, Kind: enum.UnitKindPhrase, Label: "14 July"},
			},
			Score:       0.8,
	...
	scoredNews := []appquery.ScoredNewCandidate{
		{
			Candidate: appquery.NewCandidate{
				State: model.UserUnitState{CoarseUnitID: 3, Status: enum.UnitStatusNew, TargetPriority: 0.7},
				Unit:  model.LearningUnitRef{CoarseUnitID: 3, Kind: enum.UnitKindPhrase, Label: "1 Kings"},
			},
			Score:       0.6,
		...
		{
			Candidate: appquery.NewCandidate{
				State: model.UserUnitState{CoarseUnitID: 4, Status: enum.UnitStatusNew, TargetPriority: 0.5},
				Unit:  model.LearningUnitRef{CoarseUnitID: 4, Kind: enum.UnitKindPhrase, Label: ".45 calibre"},
			},
			Score:       0.4,
...
func TestRecommendationAssemblerLetsNewFillUnusedReviewCapacity(t *testing.T) {
	...
	batch := assembler.Assemble(userID, now, nil, []appquery.ScoredReviewCandidate{
		{
			Candidate: appquery.ReviewCandidate{
				State: model.UserUnitState{CoarseUnitID: 10, Status: enum.UnitStatusReviewing},
				Unit:  model.LearningUnitRef{CoarseUnitID: 10, Kind: enum.UnitKindWord, Label: "review-only"},
			},
			Score:       0.8,
			ReasonCodes: []string{"review_due"},
		},
		...
		{
			Candidate: appquery.NewCandidate{
				State: model.UserUnitState{CoarseUnitID: 11, Status: enum.UnitStatusNew},
				Unit:  model.LearningUnitRef{CoarseUnitID: 11, Kind: enum.UnitKindPhrase, Label: "new-1"},
			},
			Score:       0.7,
			ReasonCodes: []string{"new_candidate"},
		},
		{
			Candidate: appquery.NewCandidate{
				State: model.UserUnitState{CoarseUnitID: 12, Status: enum.UnitStatusNew},
				Unit:  model.LearningUnitRef{CoarseUnitID: 12, Kind: enum.UnitKindPhrase, Label: "new-2"},
			},
			Score:       0.6,
			ReasonCodes: []string{"new_candidate"},
		},
