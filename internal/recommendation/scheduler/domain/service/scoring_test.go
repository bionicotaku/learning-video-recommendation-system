...
func TestPriorityZeroExtractorPrioritizesLearningDueAndRecentFailure(t *testing.T) {
	...
	items := []appquery.ScoredReviewCandidate{
		...
		{
			Candidate:   appquery.ReviewCandidate{State: model.UserUnitState{CoarseUnitID: 1, Status: enum.UnitStatusLearning, NextReviewAt: ptrTime(now.Add(-1 * time.Hour))}},
			Score:       0.5,
			ReasonCodes: []string{"review_due"},
		},
		{
			Candidate:   appquery.ReviewCandidate{State: model.UserUnitState{CoarseUnitID: 2, Status: enum.UnitStatusReviewing, LastQuality: &badQuality}},
			Score:       0.7,
