...
func (h *Harness) LearningEventsAPIServer(t *testing.T, userID string) *httptest.Server {
	...
	resetUserUnitProgress := learningservice.NewResetUserUnitProgressUsecase(learningtx.NewManager(h.Pool))
	...
	learningEvents := learningevents.NewHandler(
		...
		apiservice.NewResetUserUnitProgressService(resetUserUnitProgress),
	)
...
func (h *Harness) apiHandler(t *testing.T, principalMiddleware func(http.Handler) http.Handler) http.Handler {
	...
	videoLibraryReader := catalogrepo.NewVideoLibraryReader(h.Pool)
	videoLibraryHandler := videolibrary.NewHandler(
		apiservice.NewVideoLibraryService(
			catalogservice.NewListVideoFavoritesUsecase(videoLibraryReader),
			catalogservice.NewListVideoHistoryUsecase(videoLibraryReader),
			apiservice.NewPublicAssetURLBuilder("https://cdn.example.com/assets"),
		),
	)
