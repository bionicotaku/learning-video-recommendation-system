...
```text
前端在 lookup 弹窗关闭或阶段性 flush 时，把这一段已经完成的 lookup 事实写入 learning interaction 队列，再通过 learning interaction 批量 API 上报。具体请求结构见 [学习事件上报API设计.md](学习事件上报API设计.md)。
```
...
学习事件上报 API 的 endpoint、前端上传 JSON、响应结构、错误结构、字段级 validation、队列与 flush 建议，统一维护在 [学习事件上报API设计.md](学习事件上报API设计.md)。
...
| `POST /api/learning-interactions:batch` | `analytics.learning_interaction_events` | 用户发生过的学习互动事件 | 不可随意覆盖；每条 lookup / 已学会都是独立事实 | lookup、exposure、已学会等非答题学习互动分析和后续 evidence 归约 |
...
learning interaction 批量 API 是 append-only event telemetry。不同 lookup、不同“已学会”点击、不同聚合 exposure 都是需要独立解释的事实，不能像观看进度一样只保留最后一条。
...
23. learning interaction 使用批量 API 写入 `analytics.learning_interaction_events`；quiz attempt 使用单点 API 写入 `analytics.quiz_events`。
