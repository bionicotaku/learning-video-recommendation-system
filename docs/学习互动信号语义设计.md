...
lookup 是 reset 信号：如果用户在累计过程中 lookup 了这个 unit，后端会从 lookup 之后重新计算 3 个 watch session。
...
| `POST /api/learning-units:reset-unlearned` | `learning.unit_learning_events` | 用户主动把已有学习单元重置为未学习 | 每次点击是一条幂等 reducer event | reset raw-less command 和后续 `reset_unlearned` 归约 |
