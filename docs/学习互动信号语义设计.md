...
推荐系统在给前端 feed 列表时，不只是给视频列表，还会给学习推荐视频一组该用户本轮预期学习的 `learning_units`。

学习推荐视频大约有 1 到 8 个 `learning_units`。它们就是这个视频的学习重点。Recommendation 为避免 feed 空返回或数量不足追加的补全视频可以返回 `learning_units=[]`，表示该视频本轮没有学习目标。

MVP 阶段：

- 字幕自动曝光围绕“当前用户 `is_target=true` 且 `status in ('new','learning','reviewing')` 的 coarse units”处理，不再只限于 Recommendation 返回的 `learning_units`；
...
- exposure / lookup 默认不是掌握结论；同一个 unit 累计 3 个不同 watch session 的 exposure 且最近无 lookup 时，后端会生成一条 passive progress 证据；
...
```mermaid
flowchart TD
    A["Recommendation 生成 feed"] --> B["学习推荐视频带 1-8 个 learning_units；补全视频可为空"]
    B --> C["前端播放视频与字幕"]

    C --> D1["字幕播放经过当前 target/unmastered unit"]
...
因此，`learning_units` 不是视频的完整词表，而是“这个用户这次看这个视频时，系统计划高亮和出题的学习目标”。如果数组为空，前端应把它理解为普通补全视频，不触发本轮 end quiz；但 exposure 上报仍按当前用户 target/unmastered coarse unit 集合决定。

## 4. 几个角色分别负责什么

```mermaid
flowchart TD
    REC["Recommendation"] -->|决定学习推荐视频本轮学什么| PLAN["learning_units"]
...
- Recommendation：决定每个学习推荐视频本轮预期学习哪些 `learning_units`；对 video-level 补全视频返回空数组。
...
MVP 阶段，字幕 exposure 处理当前用户所有 `is_target=true` 且 `status in ('new','learning','reviewing')` 的 coarse units。

也就是：

```text
只要某个字幕 span 映射到当前用户仍在目标集合里的未掌握 coarse unit，就可以产生 exposure raw fact。
```

这不是全量 token exposure。前端仍需要先拿到当前用户 active target coarse unit 集合，并只围绕这些 unit 上报。

```mermaid
flowchart TD
    A["字幕播放经过某个 token/span"] --> B{"它属于本视频 learning_units 吗？"}
    B -->|用于高亮/quiz| C["Recommendation learning_units"]
    A --> D{"它属于当前用户 target/unmastered coarse units 吗？"}
    D -->|是| E["上报 exposure raw fact"]
    D -->|否| F["不作为 exposure 上报"]
    E --> G["后端按 watch session 聚合解释"]
```

### 5.3 exposure 要聚合

同一个视频里，同一个 unit 可能出现多次。前端可以按播放快照多次上报 raw exposure，后端语义上会按 `user_id + coarse_unit_id + watch_session_id` 去重为一次 session exposure。

稳定语义：

```text
一次 watch session 里，同一个 user + coarse_unit_id 最多计为一次 session exposure。
```

因此，同一个 watch session 内每 15 秒 flush 出来的多条 raw exposure 不会重复推进学习状态。不同 watch session 可以累加；例如视频 A 的 session 1、视频 B 的 session 2、再次观看视频 A 形成的新 session 3，可以算作 3 次 session exposure。

### 5.4 后端跨 session 解释 exposure

后端按 `user_id + coarse_unit_id` 跨 watch session 聚合 exposure。

例如：

```text
同一个 unit 在 3 个不同 watch_session_id 中被 exposure
最近一次 lookup 或 reset boundary 之后重新累计
已有 exposure_session3 progress 消费过的 watch_session_id 不再重复计入
```

这会被解释成一条 `exposure_session3_v1` passive progress 证据：
...
它会推进 progress / schedule，但不增加 quiz-style 连续成功次数。`consumed_watch_session_ids` 是后续窗口排除已消费 session 的 typed 业务来源，metadata 里的 session 列表只是审计副本。只要最终 `progress_percent >= 100`，reducer 会强制把 unit 收敛为 mastered。

lookup 是 session3 窗口 reset 信号：如果用户在累计过程中 lookup 了这个 unit，后端会从 lookup 之后重新计算 3 个 watch session。`reset_unlearned.reset_boundary_at` 也是窗口起点，并会屏蔽 boundary 前旧 raw fact。
...
| `self_mark_mastered` | 用户在 lookup 弹窗或 quiz 页面主动点“已学会”。 | 进入 Learning engine 前必须有 | normalizer 生成 `reducer_effect = 'set_mastered'` 的 normalized event，直接把该 unit 标记为已掌握；不修改 target/control 字段。 |
...
| `watch_session_id` | 前端生成的观看 session correlation key。用于说明这个互动发生在哪次观看 session 内；只校验 UUID，不要求 `analytics.video_watch_events` 已存在。 |
| `recommendation_run_id` | 如果来自某次推荐 feed 或推荐视频，记录对应 run id。MVP 不加外键，避免 analytics 和 recommendation 生命周期强耦合。 |
| `related_quiz_event_id` | 如果“已学会”按钮发生在 quiz 页面，可记录对应 quiz event id 作为来源上下文；只校验 UUID，不要求 `analytics.quiz_events` 已存在，也不表示该按钮属于 quiz attempt。 |
...
| `occurred_at` | 客户端事件发生时间。用于窗口聚合和审计；reducer replay 顺序由内部 `ledger_seq` 决定。 |
...
| --- | --- | --- | --- | --- |
| `POST /api/video-watch-progress` | `analytics.video_watch_events` + `catalog.video_user_states` | 当前观看 session 的最新进度状态 | 同一 session 内可覆盖，只保留最新进度和服务端完成状态 | 视频消费进度、完成次数、Recommendation 读取的消费投影 |
...
| `POST /api/learning-units:mark-mastered` | `analytics.learning_interaction_events` | 用户主动声明某个学习单元已掌握 | 每次点击是一条幂等 raw fact | self mark raw fact 和后续 `set_mastered` 归约；不改 target/control 字段 |
| `POST /api/learning-units:reset-unlearned` | `learning.unit_learning_events` | 用户主动把已有学习单元重置为未学习 | 每次点击是一条幂等 reducer event | reset raw-less command 和后续 `reset_unlearned` 归约；内部 `reset_boundary_at` 屏蔽旧 raw |

`watch-progress` 是 replace-latest state telemetry。前端同一 `video_id + watch_session_id` 只需要保留一个待上报状态，普通进度样本可以覆盖旧样本；完成状态由 watch-progress 后端根据累计有效播放时长和最大播放位置计算，不由前端 payload 指定。
...
```text
这个行为能不能映射到学习单元？
它是不是当前用户 target/unmastered coarse unit？
...
1. Feed 返回视频时，学习推荐视频带 `learning_units`，约 1-8 个；补全视频可以是空数组。
2. 非空 `learning_units` 是本轮视频学习计划、字幕高亮和 end quiz 候选范围。
3. exposure 上报范围是当前用户所有 `is_target=true` 且 `status in ('new','learning','reviewing')` 的 coarse units，不再只限于本轮 `learning_units`。
4. 单次 exposure 只表示“可能接触过”，不是掌握结论。
5. exposure raw 可以随播放快照多次上报；后端按 `user_id + coarse_unit_id + watch_session_id` 聚合为一次 session exposure。
6. 后端累计 3 个未消费过的不同 watch session 且最近无 lookup 时，生成一条 q4 `exposure_session3_v1` passive progress；它不计入连续成功 streak。
7. lookup 可以上报所有字幕点击。
...
15. “重置为未学习”通过 `POST /api/learning-units:reset-unlearned` 上报，只接受已有 `learning.user_unit_states` 行，直接记录为 `learning.unit_learning_events(event_type = 'reset_unlearned')`，不写 Analytics。
16. 收藏单词不是学习信号，不进入 Learning engine reducer。
17. 当前产品阶段，practice / quiz attempt 是主要 progress 反馈。
18. `legacy-new-learn` / `review` 不作为 Learning Engine 落库事件类型；attempt 来源和用户状态只作为 normalizer 上下文。
19. 非习题类原始互动进入 `analytics.learning_interaction_events`。
20. 习题答题原始事实进入 `analytics.quiz_events`。
...
```text
Recommendation 告诉前端这个视频本轮高亮什么、quiz 候选是什么。
前端如实上报用户互动。
analytics 保存原始行为。
后端把能解释的行为转成 learning evidence。
Learning engine 根据 normalized event 更新状态。
```
