...
```text
前端在 lookup 弹窗关闭或阶段性 flush 时，把这一段已经完成的 lookup 事实写入 learning interactions 队列，再通过统一 endpoint 批量上报。
```
...
| `self_mark_mastered` | 用户在 lookup 弹窗或 quiz 页面主动点“已学会”。 | 进入 Learning engine 前必须有 | 后端后续可解释成直接标记已学会的控制动作或强 evidence。 |
...
| `sentence_index` | 字幕句索引。用于定位 lookup / exposure 发生在哪一句。 |
| `span_index` | 字幕 span/token 索引。用于定位用户具体点了哪个 token/span。 |
...
```json
{
  "client_event_id": "watch_001:lookup:1",
  "event_type": "lookup",
  "source_surface": "lookup_popup",
  ...
  "sentence_index": 8,
  "span_index": 2,
  "occurred_at": "2026-05-14T20:10:01Z",
  "lookup_visible_ms": 3200,
  "lookup_sentence_audio_replay_count": 1,
  "lookup_word_audio_play_count": 0,
...
```json
{
  "client_event_id": "watch_001:mastered:1",
  ...
  "sentence_index": 8,
  "span_index": 2,
  "occurred_at": "2026-05-14T20:10:05Z"
}
```
...
```json
{
  "client_event_id": "quiz_001:mastered",
  "event_type": "self_mark_mastered",
  "source_surface": "quiz",
  "related_quiz_event_id": "8ef5f19d-2f5d-45e3-8d6e-5df96b6c7f31",
  "coarse_unit_id": 123,
  "occurred_at": "2026-05-14T20:12:01Z"
}
```
...
```json
{
  "client_event_id": "watch_001:exposure:123",
  "event_type": "exposure",
  ...
  "recommendation_run_id": "ac99d61b-f88f-4433-ad51-23d8076769bd",
  "coarse_unit_id": 123,
  "occurred_at": "2026-05-14T20:12:30Z",
  "exposure_start_ms": 15200,
  "exposure_end_ms": 17600,
  "exposure_count": 2
}
```

### 7.6 learning interactions 上报 API

`analytics.learning_interaction_events` 推荐使用一个统一语义 endpoint：

```http
POST /api/analytics/learning-interactions
```

这个 endpoint 只接收非答题 attempt 的学习互动事实：

- `exposure`
- `lookup`
- `self_mark_mastered`

它不接收 quiz answer attempt。用户真正完成一道题时，应写入 `analytics.quiz_events` 对应的答题上报链路。

请求体推荐使用批量结构，即使某次只有一个事件：
...
成功响应推荐返回 batch 级处理结果和简单计数，不需要返回每条事件的 `event_id`：
...
| 字段 | 含义 |
| --- | --- |
| `accepted` | 后端已接收并处理整个 batch。它只表示 raw analytics 事件已持久化或已幂等存在，不表示 Learning engine 已更新学习状态。 |
| `received_count` | 请求中收到的事件条数。 |
| `inserted_count` | 本次实际新插入的事件条数。 |
| `duplicate_count` | 因 `(user_id, client_event_id)` 已存在而被当作幂等重试跳过的事件条数。 |

MVP 不设计 `client_batch_id`。批量上报只是传输优化，不是业务对象；后端按每条事件的 `client_event_id` 做幂等即可。即使一次 flush 里包含多条事件，也不需要为这次 flush 分配一个可持久化的批次 ID。

前端应在 telemetry 基础层维护一个全局只读的客户端环境上下文，并在每次 learning interactions batch 上报时填入请求级 `client_context`：
...
同一个 batch 内的所有事件共享这份 `client_context`。`client_context` 只描述客户端运行环境，不描述业务入口；业务入口继续使用每条事件自己的 `source_surface`。

接口字段和表字段保持一一对应，但有两条服务端规则：

1. `user_id` 不从请求体读取，只从认证上下文获取。
2. `client_context` 是请求级字段，服务端复制到本次 batch 的每一条事件行。
3. `event_id`、`created_at` 由服务端生成。

batch 处理规则：

1. 整个 batch 先做结构和字段校验。
2. 任意事件缺少必填字段、字段类型错误或违反约束时，整批拒绝。
3. 整批校验通过后，后端批量插入。
4. 对重复的 `(user_id, client_event_id)` 使用幂等跳过，重复事件不算错误。
5. MVP 不做 partial success 响应，避免前端维护复杂的逐条 ack 状态。
...
```json
{
  "accepted": false,
  "error": "invalid_event",
  "message": "events[3].lookup_visible_ms must be >= 0"
}
```

### 7.7 前端队列与触发时机

learning interactions 是 append-only 事件事实，不是“最新状态”。因此前端可以使用轻量事件队列和批量 HTTP API。

推荐触发时机：

1. `self_mark_mastered` 发生后立即 flush，因为它代表用户明确控制意图，重要性高于普通曝光和普通 lookup。
2. lookup 弹窗关闭时入队并尽快 flush。关闭时才能知道 `lookup_visible_ms`、音频播放次数和是否点了“练一下”。
3. exposure 在一次 watch session 内先本地聚合，切换视频、退出 fullscreen、session 结束或定时 flush 时上报聚合结果。
4. 后台定时 flush，例如每 10 到 15 秒，把已完成聚合的事件批量发送。
5. 页面卸载或 fullscreen unmount 时 best-effort flush，但不能只依赖卸载。

队列语义：

- `lookup` 和 `self_mark_mastered` 不应该被后续同类事件覆盖；每次点击或声明都是独立事实。
- `exposure` 可以先在前端按 `watch_session_id + video_id + coarse_unit_id` 聚合，再形成一条事件入队。
- 每条最终上报事件必须有稳定 `client_event_id`，以支持失败重试和服务端幂等。
- MVP 不要求离线持久化；应用进程被杀导致少量低价值 exposure 丢失可以接受。
- 失败不应该 toast 打扰用户；只有未来把 `self_mark_mastered` 做成强状态写入时，才需要面向用户展示失败状态。

### 7.8 与 watch-progress API 的边界
...
| `POST /api/analytics/learning-interactions` | `analytics.learning_interaction_events` | 用户发生过的学习互动事件 | 不可随意覆盖；每条 lookup / 已学会都是独立事实 | lookup、exposure、已学会等非答题学习互动分析和后续 evidence 归约 |
| quiz attempt 上报 | `analytics.quiz_events` | 用户完成一道题的答题事实 | 一题完成后一条 completed attempt | 判题、quiz evidence、题目效果分析 |
...
`learning-interactions` 是 append-only event telemetry。不同 lookup、不同“已学会”点击、不同聚合 exposure 都是需要独立解释的事实，不能像观看进度一样只保留最后一条。

因此可以共享底层 telemetry flusher 的工程能力，例如定时 flush、unmount best-effort、失败重试和幂等 ID，但不能共享同一条业务队列合并规则，也不应该塞进同一个 HTTP endpoint。
...
- “已学会”是用户主动声明掌握状态，MVP 先记录为 `self_mark_mastered` 原始事件，后端之后可以决定是否直接标记已学会；
...
10. lookup 通过 learning interactions 上报，可以批量发送，但不塞进 watch-progress API。
11. lookup 弹窗不默认要求用户做三档自评。
12. `lookup_visible_ms`、重放全句音频、播放单词字典音，都是 lookup 附加行为，可作为解释 lookup 强度的证据。
13. “练一下”只表示触发练习，不代表学习结果。
14. lookup 弹窗和 quiz 页面里的“已学会”按钮统一记录为 `analytics.learning_interaction_events(event_type = 'self_mark_mastered')`。
15. 收藏单词不是学习信号，不进入 Learning engine reducer。
16. 当前产品阶段，practice / quiz attempt 是主要强反馈。
17. `new_learn` / `review` 不作为前端一线事件；如需使用，应由后端根据 attempt 来源和用户状态内部推导。
18. 非习题类原始互动进入 `analytics.learning_interaction_events`。
19. 习题答题原始事实进入 `analytics.quiz_events`。
20. 习题答题再由后端归约成 `quiz` 或其他强学习证据。
21. attempt 必须绑定明确的 `coarse_unit_id`，否则不进入 Learning engine。
22. 前端不上报学习结论，只上报题目答案、用时、跳过等答题事实；`self_mark_mastered` 只表示用户点了“已学会”，是否直接改变学习状态由后端决定。
23. `analytics.learning_interaction_events` 使用统一 learning interactions endpoint，上报结构优先支持 batch。
24. learning interactions 和 watch-progress 不合并 API；前者是 append-only 事件，后者是同 session 可覆盖的最新观看状态。
