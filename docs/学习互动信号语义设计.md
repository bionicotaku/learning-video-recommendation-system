...
- 字幕自动曝光围绕“当前用户 `is_target=true` 且 `status!='mastered'` 的 coarse units”处理，不再只限于 Recommendation 返回的 `learning_units`；
...
MVP 阶段，字幕 exposure 处理当前用户所有 `is_target=true` 且 `status!='mastered'` 的 coarse units。
...
3. exposure 上报范围是当前用户所有 `is_target=true` 且 `status!='mastered'` 的 coarse units，不再只限于本轮 `learning_units`。
