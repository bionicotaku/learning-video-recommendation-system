...
因此，`learning_units` 不是视频的完整词表，而是“这个用户这次看这个视频时，系统关心的学习目标”。如果数组为空，前端应把它理解为普通补全视频，不产生 target exposure，也不触发本轮 end quiz。
...
MVP 阶段，字幕 exposure 只处理当前视频的非空 `learning_units`。
...
2. 非空 `learning_units` 是本轮视频学习的主范围。
3. exposure 只处理当前视频的非空 `learning_units`。
