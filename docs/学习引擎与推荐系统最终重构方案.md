...
重构开始前，仓库中的 `internal/recommendation/scheduler` 实际上混合了两类职责：
...
这次方案已按“不要向后兼容”的标准落地，不保留旧边界。

这里的“不要向后兼容”不仅指代码接口，也包括 migration 和数据库对象：

- 不保留旧 `scheduler` 目录下的 migration 作为历史兼容层
- 不保留“先迁到新模块，但旧表/旧 schema/旧列先留着”的拖尾方案
- 不保留双 owner 的数据库对象

当前已经达成：

- Learning engine 有自己干净的 migration 根
- Recommendation 有自己干净的 migration 根
- 旧 `scheduler` 目录已从仓库删除
- 仓库中不再存在一个“混合旧结构 + 新结构”的 migration 目录
...
这次重构不能只拆代码，还必须把 migration 一起拆干净。

### 7.1 最终只能有两套 migration

最终仓库中应只保留两套有效 migration：

1. Learning engine migration
2. Recommendation migration

当前仓库内已经不再保留旧 `scheduler` migration 根。

### 7.2 旧 migration 下新增的数据库对象必须删除

旧 `scheduler` migration 曾创建或维护过以下对象：
...
- 新库直接使用 Learning engine / Recommendation 两套 migration
- 旧库通过一次性脚本清理旧对象
- 不再保留“旧 migration 文件里还定义这些对象，但新 migration 也定义一次”的双份结构

### 7.3 最终 migration 必须是干净 owner

这里的“干净”具体指：

- Learning engine migration 只定义 Learning engine 自己的 schema 与表
- Recommendation migration 只定义 Recommendation 自己的 schema 与表
- 不出现一个 migration 目录里跨模块定义别人的表
- 不出现旧目录里还残留无主对象

### 7.4 `last_recommended_at` 的迁移必须是硬切换

`last_recommended_at` 的迁移不允许采用以下方案：

- 同时在 `learning.user_unit_states` 和 `recommendation.user_unit_serving_states` 保留一段时间双读
- 旧字段只标记弃用但不删除
- 代码已经改到新表，但数据库还把旧列长期留着
...
### 7.5 `scheduler_runs / scheduler_run_items` 的 owner 必须改正

这两张表当前如果还在 `learning` schema 下，只能视为历史遗留。

最终要求是：

- Recommendation 审计表归 Recommendation migration 定义
- Learning engine migration 中不再出现它们
...
因为要求“不做向后兼容”，现有数据库的迁移方案采用一次性硬切换。

### 第一步：应用两套新 migration 根

- Learning engine migration
- Recommendation migration

### 第二步：回填 Recommendation serving state

执行：

- [backfill_recommendation_serving_states.sql](/Users/evan/Downloads/learning-video-recommendation-system/scripts/legacy/backfill_recommendation_serving_states.sql)

### 第三步：删除旧 scheduler 对象
