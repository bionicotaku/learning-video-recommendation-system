...
本文档不再描述已经删除的旧 `internal/recommendation/scheduler`。
...
- `domain` 放推荐规则
- `application` 放用例编排
- `infrastructure` 放 SQL 和落库
- `test` 证明 Recommendation 只读 Learning engine、只写 Recommendation
...
```mermaid
flowchart TD
    A["GenerateLearningUnitRecommendationsUseCase"] --> B["UserUnitStateReadRepository"]
...
- [internal/recommendation/domain/service](/Users/evan/Downloads/learning-video-recommendation-system/internal/recommendation/domain/service)
...
- [internal/recommendation/test/integration](/Users/evan/Downloads/learning-video-recommendation-system/internal/recommendation/test/integration)
...
- 删除旧 scheduler 测试
- 删除 Recommendation 对 Learning engine 的写路径

保留的唯一“旧结构文档”只有：

- [学习引擎与推荐系统重构基线盘点.md](/Users/evan/Downloads/learning-video-recommendation-system/docs/学习引擎与推荐系统重构基线盘点.md)

它是历史基线记录，不再代表当前代码结构。

## 10. 现有数据库的遗留清理

对于已经跑过旧 scheduler migration 的数据库，当前仓库提供了两个一次性 SQL：
...
4. 最后执行 `drop_old_scheduler_objects.sql`
...
3. [generate_recommendations.go](/Users/evan/Downloads/learning-video-recommendation-system/internal/recommendation/application/usecase/generate_recommendations.go)
4. `domain/service/*`
