...
| 模块 | 职责 | 不做什么 |
| --- | --- | --- |
| `internal/api` | HTTP handler、请求 validation、调用 Recommendation 和 Catalog/semantic 读取能力、组装 FeedResponse。 | 不写 SQL，不拥有业务表，不实现推荐排序规则。 |
...
| semantic unit 读取能力 | 提供按 `coarse_unit_id[]` 批量读取 `semantic.coarse_unit.label`。 | 不参与推荐排序或题目选择。 |
...
| `preferred_duration_sec.min` | integer | 否 | `45` | 偏好的最短视频时长，单位秒。 |
| `preferred_duration_sec.max` | integer | 否 | `180` | 偏好的最长视频时长，单位秒。小于 `min` 时整组回退默认值。 |
...
| `cover_image_url` | string \| null | `catalog.videos.thumbnail_url` | 视频封面地址；为空时前端显示 fallback。 |
...
MVP Feed API 对前端返回的 `FeedLearningUnit` 要求 evidence 字段完整。若 Recommendation 返回某个 unit 但 evidence 缺失，说明 Catalog evidence 链路存在数据质量问题。Feed facade 应跳过该 unit，并记录服务端 warning；若某个 video 过滤后没有任何 `learning_units`，则该 video 不应出现在 feed 响应中。
...
| `cover_image_url` | `catalog.videos.thumbnail_url` |
...
MVP 不把“推荐结果少于请求数量”作为错误。Recommendation 自己会记录 `underfilled` 到 audit；Feed API 返回实际可展示 items。
