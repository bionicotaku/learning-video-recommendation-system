...
- [Legal-Documents-API-MVP设计.md](Legal-Documents-API-MVP设计.md)：公开读取隐私政策和用户协议；该 GET endpoint 不要求用户 JWT 鉴权，内容存储在 `app_user.legal_documents`。
...
当前 `internal/api` 已实现 19 个业务 HTTP endpoint。实现口径以 handler route registration 为准：
