...
文档状态：总体设计规范，当前尚未实现 `internal/api`
目标读者：后端、前端、后续接手维护的人
当前范围：定义未来 `internal/api` 作为统一流量入口时的 owner 边界、目录结构、请求处理规范、跨模块调用规则、错误响应规范和测试要求。
...
- `middleware/`：request id、panic recovery、timeout、logging、CORS、body size limit。
...
- [学习事件上报API设计.md](学习事件上报API设计.md) 定义 `POST /api/learning-interactions:batch` 和 `POST /api/quiz-attempts` 的字段、样例与业务语义。
- 本文规定这两个 endpoint 在 `internal/api` 中应该如何落目录、如何认证、如何 validation、如何调用 Analytics / Learning Engine、如何统一错误响应。
