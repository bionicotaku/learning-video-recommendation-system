# Unit Progress API MVP 设计

## 0. 文档信息

文档名称：Unit Progress API MVP 设计
适用阶段：MVP
...
1. 已掌握词列表：读取所有已经 mastered 的词，并按 `label` 的字典序分页。
...
| `last_progress_at` | string \| null | `learning.user_unit_states.last_progress_at` | 最近一次推进 progress 或 self-mark mastered 的学习进度时间。新词可能为空。 |
...
- `limit < 1`：返回参数错误。
- `limit > 100`：返回 `400 invalid_request`，不静默截断。
- `cursor` 解码失败：返回参数错误。
...
1. 前端可以分页读取 mastered units，顺序稳定为 `label` 字典序。
