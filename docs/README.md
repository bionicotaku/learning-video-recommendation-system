...
除上述四份外，[API/](API/README.md) 下的 API 设计文档与 `编码和结构规范.md` 是跨模块工程规范；运行时现状快照见本文后面的“运行时现状参考”。其余历史设计稿、讨论稿、对比稿均视为非权威参考材料，不可作为当前实现依据。
...
API 设计文档目录。包含未来 `internal/api` 的统一入口规范，以及学习事件上报、观看进度上报、学习进度读取等具体 API 契约草案。
