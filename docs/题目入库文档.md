...
`analytics.quiz_events` 不直接改变学习状态，不保存 `quality`，也不代表用户已经掌握某个学习单元。
...
本地 Catalog ingest 从每个 clip 同名 question JSON 的 `questions[]` 写入 `catalog.questions`。question JSON 文件必须与父 clip 推导出的 transcript 文件同名，且必须和 transcript 同时存在。
...
- 用户最终掌握程度是多少；
- 这次答题应该给 Learning engine 多少 quality；
- 下次什么时候复习；
- 是否应该影响推荐排序。
...
是否写入、写入时 `quality` 如何计算，不在本文定稿。MVP 可以先只保证 analytics 原始事实完整、可追溯。
...
4. 插入 `analytics.quiz_events`；
5. 使用 `(user_id, client_event_id)` 保证幂等。
