...
对于跑过旧 scheduler migration 的数据库，Learning engine 当前依赖以下一次性清理顺序：

1. 先应用新的 Learning engine migration
2. 再应用新的 Recommendation migration
3. 执行 [backfill_recommendation_serving_states.sql](/Users/evan/Downloads/learning-video-recommendation-system/scripts/legacy/backfill_recommendation_serving_states.sql)
4. 执行 [drop_old_scheduler_objects.sql](/Users/evan/Downloads/learning-video-recommendation-system/scripts/legacy/drop_old_scheduler_objects.sql)

其中与 Learning engine 直接相关的是：

- 删除 `learning.user_unit_states.last_recommended_at`
- 删除旧 `learning.scheduler_runs`
- 删除旧 `learning.scheduler_run_items`
- 删除旧 `learning.user_scheduler_settings`
