...
状态：DEPRECATED
说明：本文档仅保留为历史讨论记录，不再作为当前系统实现依据。当前实现以《视频推荐系统总设计.md》《学习引擎设计.md》《推荐模块设计.md》《Catalog-数据库设计.md》为准。
...
3. Recommendation 的当前实现范围与后续演进方向是什么
4. 系统对外最终返回什么，以及当前代码实际返回什么
...
- 这是系统目标形态
- 当前代码已落地的 Recommendation 只实现到 scheduler 批次，不包含视频召回层和学习任务层
...
而是只调用 Recommendation 模块，获取 Recommendation 当前阶段的统一输出。
...
### 3.3 Recommendation 的当前实现与后续演进

当前已落地的 Recommendation 子能力只有一层：
...
这些内部层次仅用于 Recommendation 内部组织，不构成对上层暴露的独立服务。
...
- 生成当前 Recommendation 输出

当前代码中的 Recommendation 输出是：
...
### Recommendation 内部当前包含

- 学习内容调度器

### Recommendation 内部后续可扩展为
...
4. Recommendation 的后续子层仍可独立迭代
5. 上层业务 API 不需要感知 Recommendation 内部流水线

## 5. 整体系统架构

当前 MVP 建议由 5 份核心文档共同描述：
...
### 5.2 Recommendation 内部数据流（当前实现）

```text
learning.user_unit_states + semantic.coarse_unit + recommendation.user_unit_serving_states
                ↓
          [学习内容调度器]
                ↓
      输出：推荐批次
...
指当前代码中由 Recommendation scheduler 输出的、本轮应优先推荐的一组学习内容结果。

它是当前代码真正对外可交付的 Recommendation 输出。
...
这是 Recommendation 后续扩展到完整三层流水线后，scheduler 层的中间结果。

它不是当前代码的最终输出，而是未来 `scheduler -> recall -> task` 流水线中的中间表示。
...
学习任务是 Recommendation 模块目标形态下最终输出给上层业务 API 的结果实体。
...
## 7.3 Recommendation 当前已实现范围

当前代码已经实现的 Recommendation 范围只有：
...
## 8.4 Recommendation 当前输出阶段

当前 Recommendation 在调度阶段完成后，直接输出推荐批次，并同步写入：
...
## 8.7 Recommendation 审计与 serving state 更新
...
虽然 Recommendation 的目标形态存在“学习内容调度”和“视频召回”，但从产品视角看，最终推荐对象应是学习任务，而不是单独的学习内容项，也不是纯视频内容。

### 10.2 当前代码里推荐批次是最终输出，学习内容列表是后续中间表示层

当前代码里，对外输出的是推荐批次。

后续如果扩展到完整三层流水线，学习内容列表的作用才是作为：
...
### 10.4 区分当前代码输出与目标形态输出

当前代码里，Recommendation 对上层业务 API 的主返回结果是推荐批次。

目标形态里，Recommendation 对上层业务 API 的主返回结果应是最终学习任务。
...
- 设计稿：学习事件、学习状态、SM-2、状态迁移、replay
- 工程稿：目录结构、reducer、repository、SQL、migration、测试
...
- Recommendation 当前只实现 scheduler，后续可扩展到“学习内容调度器 -> 视频召回层 -> 学习任务层”的三层流水线
- 当前代码对外返回的是推荐批次；系统目标形态对外返回的是学习任务
