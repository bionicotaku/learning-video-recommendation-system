# 学习互动信号架构图

## 0. 文档信息

文档状态：MVP 架构图草案

目标读者：产品、前端、后端、数据、后续接手维护的人

当前范围：本文只用图说明学习互动信号链路中需要设计哪些组件、组件之间怎么协作、哪些数据进入 Learning engine。本文会标出未来 analytics 原始流水的逻辑表名，但不定义具体字段、索引、migration、API、队列、埋点 SDK 或代码目录。

配套阅读：

- [学习互动信号语义设计.md](学习互动信号语义设计.md)：解释 exposure、lookup、lookup 弹窗附加行为、习题答题这些信号的业务语义。
- [练习题绑定与生成设计.md](练习题绑定与生成设计.md)：解释习题生成、触发和题目形态。

## 1. 设计目标

学习互动信号架构要解决一个核心问题：

> 前端看到的是播放、字幕点击、弹窗按钮、答题这些用户行为；Learning engine 需要的是可以归约到 `user_id + coarse_unit_id` 的 learning evidence。

因此架构必须把两件事分开：

- **原始互动事实**：用户实际做了什么，未来应该进入 analytics。
- **learning evidence**：后端确认可以绑定学习单元、可供 reducer 使用的学习证据。

```mermaid
flowchart LR
    A["用户互动事实"] --> B["Analytics 原始日志"]
    A --> C["后端标准化和分流"]
    C --> D{"能绑定学习单元?"}
    D -->|能| E["Learning evidence"]
    D -->|不能| B
    E --> F["后续 reducer 决定是否更新状态"]
```

## 2. 简化架构图

最需要理解的主链路。

```mermaid
flowchart TD
    REC["Recommendation"] -->|返回视频和 learning_units| FE["Frontend"]
    FE -->|播放、lookup、音频、答题事实| ING["Interaction Ingestion"]

    ING --> RAW1["analytics.learning_interaction_events"]
    ING --> RAW2["analytics.quiz_events"]
    ING --> NORM["Interaction Normalizer"]

    NORM -->|learning evidence| LE["Learning engine"]

    PRACTICE["Practice / Exercise"] -->|题目| FE
    CATALOG["Catalog / Semantic"] -->|字幕、span、unit 映射| NORM
```

简化理解：

- Recommendation 告诉前端这个视频本轮要学哪些 `learning_units`。
- Frontend 只上报用户互动事实，不判断用户是否学会。
- Interaction Ingestion 是统一入口，先保留原始事实。
- 非习题类原始互动未来保存到 `analytics.learning_interaction_events`。
- 习题 / 练习答题原始事实未来保存到 `analytics.quiz_events`。
- Interaction Normalizer 判断哪些事实能转成 learning evidence。
- Learning engine 只接收可归约的 learning evidence。
- Practice / Exercise 负责题目，答题 attempt 原始事实进入 analytics，后续再归约成 `quiz` 或其他强学习证据。

## 3. 详细设计图

这张图展示 MVP 阶段大致需要设计的组件。它是架构分解，不是代码包名。

```mermaid
flowchart TD
    subgraph planning["学习计划来源"]
        REC["Recommendation"]
        PLAN["video learning_units"]
    end

    subgraph frontend["Frontend"]
        PLAYER["Video Player"]
        SUBTITLE["Clickable Subtitle"]
        POPUP["Lookup Popup"]
        QUIZUI["Quiz UI"]
        COLLECTOR["Interaction Collector"]
        SESSION["Watch Session Aggregator"]
        ATTEMPT["Practice Attempt"]
    end

    subgraph backend["互动接入和标准化"]
        INGEST["Interaction Ingestion"]
        RAWROUTER["Raw Event Router"]
        RESOLVER["Unit Resolver"]
        ROUTER["Signal Router"]
        EXPOSURE["Exposure Reducer"]
        LOOKUP["Lookup Mapper"]
        AUDIO["Audio Action Mapper"]
        QUIZ["Quiz Result Mapper"]
        PRACTICETRIGGER["Practice Trigger Router"]
    end

    subgraph analytics["Analytics raw storage"]
        RAWI["analytics.learning_interaction_events"]
        RAWQ["analytics.quiz_events"]
    end

    subgraph learning["Learning domain"]
        EVENT["Learning Evidence Writer"]
        LE["Learning engine"]
        STATE["User Unit State"]
    end

    CATALOG["Catalog"]
    SEMANTIC["Semantic Unit Mapping"]
    PRACTICE["Practice / Exercise"]
    QUESTION["Question Bank"]

    REC --> PLAN
    PLAN --> PLAYER
    PLAN --> SESSION
    PLAYER --> SUBTITLE
    SUBTITLE --> POPUP
    PLAYER --> QUIZUI

    POPUP --> COLLECTOR
    SUBTITLE --> COLLECTOR
    QUIZUI --> COLLECTOR
    SESSION --> COLLECTOR

    CATALOG --> RESOLVER
    SEMANTIC --> RESOLVER
    PRACTICE --> QUESTION
    QUESTION --> QUIZUI
    QUIZUI --> ATTEMPT
    ATTEMPT --> COLLECTOR

    COLLECTOR --> INGEST
    INGEST --> RAWROUTER
    RAWROUTER --> RAWI
    RAWROUTER --> RAWQ
    INGEST --> RESOLVER
    RESOLVER --> ROUTER

    ROUTER --> EXPOSURE
    ROUTER --> LOOKUP
    ROUTER --> AUDIO
    ROUTER --> QUIZ
    ROUTER --> PRACTICETRIGGER

    EXPOSURE --> EVENT
    LOOKUP --> EVENT
    AUDIO --> EVENT
    QUIZ --> EVENT
    PRACTICETRIGGER --> RAWI
    EVENT --> LE
    LE --> STATE
```

## 4. 组件职责说明

### 4.1 Recommendation

Recommendation 的职责是给 feed 中每个视频附带 `learning_units`。

`learning_units` 不是视频完整词表，而是本轮这个用户看这个视频时，系统真正关心的学习单元。前端可以基于它做字幕高亮、exposure 聚合、末尾小测候选。

### 4.2 Frontend

Frontend 的职责是展示视频、字幕、弹窗和题目，并如实上报用户发生了什么。

前端不负责判断：

- 用户是否学会；
- 互动事实应该形成哪类 learning evidence；
- `quiz` evidence 的 quality 应该是多少；
- 这个 evidence 是否应该推进学习状态。

这些都由后端标准化层和 Learning engine 处理。

### 4.3 Interaction Collector

Interaction Collector 是前端侧的互动事实收集器。它收集：

- 字幕曝光候选；
- 字幕 token lookup；
- lookup 弹窗内重放全句音频、播放单词音；
- 点“练一下”的触发事实；
- 题目答题事实；
- watch session 中聚合后的 exposure 信息。

它的输出是互动事实，不是学习结论。

### 4.4 Watch Session Aggregator

Watch Session Aggregator 负责把播放过程中的 exposure 降噪。

MVP 语义是：

```text
一次观看 session 里，一个 video + coarse_unit_id 最多形成一次 exposure 候选。
```

它只围绕 Recommendation 返回的 `learning_units` 聚合，不对视频里所有 token 做全量 exposure。

### 4.5 Interaction Ingestion

Interaction Ingestion 是后端统一接入层。

它应该先做两件事：

1. 保存 raw interaction log，保证产品分析和排障有原始事实。
2. 把互动事实交给后端标准化链路，判断是否能形成 learning evidence 或练习触发审计。

这个层不应该直接更新学习状态。

### 4.6 Raw Interaction Log

Raw Interaction Log 是 analytics 数据。

未来按两张 analytics 表拆分：

- `analytics.learning_interaction_events`：保存 exposure、lookup、lookup 音频动作、练一下触发等非习题类互动原始事实。
- `analytics.quiz_events`：保存习题 / 练习答题原始事实。

它们保存用户实际做过什么，包括不能映射到学习单元的行为，例如 unmapped lookup。它们不直接改变学习状态。

当前状态：这是未来落库方向，当前仓库还没有对应 migration。

### 4.7 Unit Resolver

Unit Resolver 负责把前端上报的 token、span、question、attempt 等事实映射到 `coarse_unit_id`。

如果无法得到明确 `coarse_unit_id`，这个行为不能进入 Learning engine，只能留在 raw interaction log 或 attempt 里。

### 4.8 Signal Router

Signal Router 负责把已经解析过的互动事实分到不同处理器：

- exposure 交给 Exposure Reducer；
- lookup 交给 Lookup Mapper；
- lookup 弹窗内音频动作交给 Audio Action Mapper；
- 习题答题交给 Quiz Result Mapper；
- “练一下”触发交给 Practice Trigger Router。

```mermaid
flowchart LR
    A["Resolved Interaction"] --> B{"signal type"}
    B -->|exposure| C["Exposure Reducer"]
    B -->|lookup| D["Lookup Mapper"]
    B -->|audio action| E["Audio Action Mapper"]
    B -->|quiz answer| F["Quiz Result Mapper"]
    B -->|practice trigger| G["Practice Trigger Router"]
```

### 4.9 Exposure Reducer

Exposure Reducer 把聚合后的 exposure 候选解释成 exposure evidence。

单次 exposure 只表示“用户可能接触过这个 unit”，不表示用户理解或掌握。跨视频多次 exposure 是否形成 passive familiarity，由后续 reducer 规则决定。

### 4.10 Lookup Mapper

Lookup Mapper 把 mapped lookup 转成 active interest / possible difficulty 这类 lookup evidence。

unmapped lookup 只留在 Raw Interaction Log，不进入 Learning engine。

### 4.11 Audio Action Mapper

Audio Action Mapper 处理 lookup 弹窗中的音频动作：

- 重放全句音频；
- 播放单词字典音。

这些动作可以帮助解释 lookup 强度，但不直接代表用户掌握。

### 4.12 Quiz Result Mapper

Quiz Result Mapper 处理习题答题。

答题原始事实先保存到 `analytics.quiz_events`。Quiz Result Mapper 再根据题目答案、题型、难度和答题结果，把 attempt 归约成 `quiz` 或其他强学习证据。

```mermaid
sequenceDiagram
    participant FE as Frontend
    participant Practice as Practice
    participant Analytics as Analytics
    participant Mapper as Quiz Result Mapper
    participant LE as Learning engine

    FE->>Practice: 提交答题事实
    Practice->>Analytics: 保存 analytics.quiz_events
    Practice->>Mapper: 交给后端判题和归约
    Mapper->>Mapper: 计算 is_correct 和 quality
    Mapper->>LE: 写入 quiz evidence
```

### 4.13 Practice Trigger Router

Practice Trigger Router 处理“练一下”这类触发动作。

它不是学习结果，不应该直接写成 `quiz`、`review` 或 `lookup`。真正的学习反馈来自后续答题 attempt 和归约出来的 quiz evidence。

### 4.14 Learning Evidence Writer

Learning Evidence Writer 是进入 Learning engine 前的最后一层。

它只写可归约 learning evidence，例如：

- `exposure`
- `lookup`
- `quiz`

进入这里的数据必须已经有明确 `user_id + coarse_unit_id`。

### 4.15 Learning engine

Learning engine 只负责归约学习状态。

它不关心：

- 前端如何展示弹窗；
- 题目如何生成；
- 用户点击的是哪个原始 token；
- analytics 保存了哪些 raw log。

它只处理可归约 learning evidence，并由 reducer 决定是否更新 `user_id + coarse_unit_id` 对应的学习状态。

## 5. 关键信号流

### 5.1 exposure 流

```mermaid
flowchart LR
    A["字幕播放经过 learning_unit"] --> B["Watch Session Aggregator"]
    B --> C["Interaction Ingestion"]
    C --> D["analytics.learning_interaction_events"]
    C --> E["Exposure Reducer"]
    E --> F["Exposure evidence"]
```

### 5.2 lookup 流

```mermaid
flowchart LR
    A["用户点击字幕 token"] --> B["Interaction Ingestion"]
    B --> C["analytics.learning_interaction_events"]
    B --> D["Unit Resolver"]
    D --> E{"能映射 coarse_unit_id?"}
    E -->|能| F["Lookup evidence"]
    E -->|不能| G["仅保留 raw log"]
```

### 5.3 lookup 弹窗附加行为流

```mermaid
flowchart TD
    A["lookup 弹窗动作"] --> B{"动作类型"}
    B -->|重放全句 / 播放单词音| C["analytics.learning_interaction_events"]
    C --> D["Audio Action Mapper"]
    D --> E["lookup 强度证据"]

    B -->|练一下| F["Practice Trigger Router"]
    F --> G["触发 quiz session"]
```

### 5.4 quiz 流

```mermaid
flowchart TD
    A["用户完成题目"] --> B["Practice Attempt"]
    B --> C["analytics.quiz_events"]
    B --> D["Quiz Result Mapper"]
    D --> E{"题目绑定 coarse_unit_id?"}
    E -->|是| F["Quiz evidence"]
    E -->|否| G["只保留 attempt"]
```

## 6. 新人读图重点

读这个架构时，优先抓住四条边界：

1. Frontend 上报事实，不上报学习结论。
2. `analytics.learning_interaction_events` 和 `analytics.quiz_events` 保存原始行为，不更新学习状态。
3. Learning engine 只接收有明确 `coarse_unit_id` 的 learning evidence。
4. Practice attempt 和 Learning engine `quiz` evidence 是两层语义。

如果某个组件或接口设计无法回答“这条数据是原始事实、learning evidence、还是触发/控制动作”，说明边界还没有设计清楚。
